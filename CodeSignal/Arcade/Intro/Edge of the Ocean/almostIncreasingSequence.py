...
"""
def check_increasing_monotonicity(arr):
    return all(arr[i] < arr[i + 1] for i in range(len(arr) - 1))

def almostIncreasingSequence(sequence):
    for i in range(len(sequence) - 1):
        if sequence[i] >= sequence[i + 1]:
            return check_increasing_monotonicity(sequence[:i] + sequence[i + 1:]) or check_increasing_monotonicity(sequence[:i + 1] + sequence[i + 2:])
