...
def atoi(s):
    ...
    for ch in s[1:]:
        if ch.isnumeric(): ans += ch
        else: break
    
    #Very Important edge cases: s = "3.14176" and s = "-"
    return 0 if len(ans) == 1 and not ans.isnumeric() else max(-2**31, min(int(ans), 2**31-1))
...
class TestStringToIntegerAtoi(unittest.TestCase):
    ...
    def test_generic(self):
        ...
        self.assertEqual(atoi("3.14159"), 3)
